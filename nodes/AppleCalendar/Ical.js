		...
		if (curr.uid) {
			...
			if (curr.recurrenceid != null) {
				...
				if (typeof curr.recurrenceid.toISOString === 'function') {
					par[curr.uid].recurrences[curr.recurrenceid.toISOString().substring(0, 10)] =
						recurrenceObj;
				} else {
...
function handleObject(name, val, params, ctx, stack, line) {
	if (objectHandlers[name]) return objectHandlers[name](val, params, ctx, stack, line);
...
export function parseICS(str) {
	var line_end_type = getLineBreakChar(str);
